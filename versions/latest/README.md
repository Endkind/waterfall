...
```bash
docker run -it -d -p 25565:25577 --name endkind-waterfall endkind/waterfall:1.21
```
...
docker run -it -d -p 25565:25577 --name endkind-waterfall -v endkind-waterfall:/waterfall --restart=always endkind/waterfall:1.21
```
...
```bash
docker build -t endkind/waterfall:latest .
```
